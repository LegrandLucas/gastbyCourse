...
const createHash = (matchPaths, components, cleanedClientVisitedPageComponents, notVisitedPageComponents) => _crypto.default.createHash(`md5`).update(JSON.stringify({
  matchPaths,
  components,
...
const writeAll = async state => {
  ...
  let cleanedClientVisitedPageComponents = components;
  let notVisitedPageComponents = components;

  if (process.env.GATSBY_EXPERIMENTAL_LAZY_DEVJS) {
    const clientVisitedPageComponents = [...state.clientVisitedPages.values()]; // Remove any page components that no longer exist.
  ...
  const newHash = createHash(matchPaths, components, cleanedClientVisitedPageComponents, notVisitedPageComponents);
...
\n\n`;
  ...
  (0, _gatsbyWebpackVirtualModules.writeModule)(`$virtual/ssr-sync-requires`, syncRequires);
...
if (process.env.GATSBY_EXPERIMENTAL_LAZY_DEVJS) {
  ...
  _redux.emitter.on(`CREATE_CLIENT_VISITED_PAGE`, () => {
    ...
    writeAll(_redux.store.getState());
  });
}
